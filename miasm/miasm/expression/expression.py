...
class ExprInt(Expr):
    ...
    def toSigned(self):
        if self.size == 8:
            return ctypes.c_int8(self.arg).value
        elif self.size == 16:
            return ctypes.c_int16(self.arg).value
        elif self.size ==32:
            return ctypes.c_int(self.arg).value
        elif self.size == 64:
            return ctypes.c_long(self.arg).value
        elif self.size == 128:
            return ctypes.c_longlong(self.arg).value

    def toUSinged(self):
        if self.size == 8:
            return ctypes.c_uint8(self.arg).value
        elif self.size == 16:
            return ctypes.c_uint16(self.arg).value
        elif self.size == 32:
            return ctypes.c_uint(self.arg).value
        elif self.size == 64:
            return ctypes.c_ulong(self.arg).value
        elif self.size == 128:
            return ctypes.c_longlong(self.arg).value
...
class ExprCond(Expr):
    ...
    def __str__(self):
        if isinstance(self._cond, ExprSlice) and (self._cond.start == 31) and (self._cond.stop == 32) and (isinstance(self._src1, ExprCompose)) and (isinstance(self._src2, ExprCompose)) and (self._src1.args[0] == self._src2.args[0]):
            return str(self._src1.args[0])
...
class ExprOp(Expr):
    ...
    def __str__(self):
        ...
        if self.is_associative() or self.is_infix():
            if (len(self._args) == 2) and (isinstance(self._args[0], ExprId)) and (isinstance(self._args[1], ExprInt)):
                return (str(self._args[0]) + ' ' + self._op + ' ' + self._args[1].toSignedStr())
            else:
                return (' ' + self._op +' ').join([str_protected_child(arg, self) for arg in self._args])
        return (self._op + '(' +
...
class ExprCompose(Expr):
    ...
    def __str__(self):
        return '(' + ', '.join(["%s" % (arg) for arg in self._args if not isinstance(arg, ExprInt)]) + ')'
